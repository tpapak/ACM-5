...
# ACM_5_with_networks <- read_excel("data/ACM-5%-with-networks-with-Argos.xlsx")
...
studyGraphs <- lapply(studyIds, function(stid){
  ...
  },warning = function(wmes){
    warnmess <<- c(warnmess,wmes$message)
    stgr <- fullGraph(stid, studyTable ,rand=F)
    studyInconsistency <<- rbind(studyInconsistency
                                , list(id=stgr$study
                                      , Q=stgr$Q
...
moreIncs <- studyInconsistency[order(studyInconsistency$pv, decreasing = F),]
moreIncs
print(warnmess)
```
...
The following studies present inconsistency in their relative effects: `r moreIncs[moreIncs$Q>0,]$id`. Specifically the first `r nrow(moreIncs[moreIncs$Q>0 & moreIncs$pv<0.95,])` should be checked.
